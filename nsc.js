...
class Node {
  constructor() {
    ...
    this.col = Math.random() > 0.5 ? "124,58,237" : "6,182,212";
  }
  update() {
    this.x += this.vx;
    this.y += this.vy;
  ...
  draw() {
    ...
    ctx.fillStyle = `rgba(${this.col},.75)`;
    ctx.fill();
  }
}
  ...
  for (let i = 0; i < nodes.length; i++)
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (d < 140) {
        ...
        ctx.strokeStyle = `rgba(124,58,237,${(1 - d / 140) * 0.22})`;
        ctx.lineWidth = 0.5;
        ctx.stroke();
      }
    }
...
function animateCounters() {
  document.querySelectorAll(".stat-val").forEach((el) => {
    const html = el.innerHTML,
      num = parseFloat(html.replace(/<[^>]+>/g, "").replace(/[^0-9.]/g, ""));
    if (!num) return;
    let start = 0;
  ...
  (e) => {
    e.forEach((x) => {
      if (x.isIntersecting) {
        ...
        if (x.target.closest(".stats-band")) animateCounters();
      }
    });
  },
  { threshold: 0.12 },
